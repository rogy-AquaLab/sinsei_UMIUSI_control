...
auto suc_util::LinuxCan::close() -> tl::expected<void, std::string> {
    ...
    if (!res_tx || !res_rx) {
        if (!res_tx) {
            return tl::make_unexpected("Failed to close TX socket: " + res_tx.error());
        }
        return tl::make_unexpected("Failed to close RX socket: " + res_rx.error());
    }
...
auto suc_util::LinuxCan::init(const std::string ifname) -> tl::expected<void, std::string> {
    ...
    if (!res_tx || !res_rx) {
        // If RX initialization fails, close the TX socket before returning
        if (!res_tx) {
            return tl::make_unexpected("Failed to initialize TX socket: " + res_tx.error());
        }
        return tl::make_unexpected("Failed to initialize RX socket: " + res_rx.error());
    }
