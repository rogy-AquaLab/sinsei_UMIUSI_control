...
auto ImuModel::on_init() -> tl::expected<void, std::string> {
    ...
    res = this->i2c->write_reg(BNO055_ADDR, OPR_MODE_ADDR, OPERATION_MODE_CONFIG);
    if (!res) {
        return tl::make_unexpected("Failed to set CONFIG mode: " + res.error());
    }

    // リセット
    res = this->i2c->write_reg(BNO055_ADDR, SYS_TRIGGER_ADDR, std::byte{0x20});
    if (!res) {
    ...
    for (int i = 0; i < 100; ++i) {  // 10ms * 100 = 1000ms
        ...
        if (current_id && current_id.value() == this->BNO055_ID) {
            timeout = false;
            break;
        }
    ...
    // NDOFモードに設定
    res = this->i2c->write_reg(BNO055_ADDR, OPR_MODE_ADDR, OPERATION_MODE_NDOF);
...
auto ImuModel::on_read() -> tl::expected<
    ...
    auto res = this->i2c->read_reg(
        BNO055_ADDR, interface::I2cRegisterAddr{FRAME_START},
