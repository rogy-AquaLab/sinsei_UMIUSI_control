...
auto ImuModel::write_reg(RegisterAddr reg, std::byte value) -> tl::expected<void, std::string> {
...
auto ImuModel::read_reg(RegisterAddr reg, interface::I2cBufferView buffer)
...
auto ImuModel::validate_id() -> tl::expected<void, std::string> {
    std::byte value{};
...
auto ImuModel::wait_for_device_ready(int max_attempts, std::chrono::milliseconds retry_interval)
    -> tl::expected<void, std::string> {
    if (max_attempts <= 0) {
...
auto ImuModel::reset_device() -> tl::expected<void, std::string> {
...
auto ImuModel::configure_device() -> tl::expected<void, std::string> {
    // ノーマルパワーモードに設定
...
auto ImuModel::on_read() -> tl::expected<ImuModel::ReadResult, std::string> {
