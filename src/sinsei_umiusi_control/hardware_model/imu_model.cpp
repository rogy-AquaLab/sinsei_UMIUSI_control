...
auto ImuModel::on_init() -> tl::expected<void, std::string> {
    ...
    for (int time = 0; time < TIMEOUT_MS; time += WAIT_INTERVAL_MS) {
        auto chip_id_res = this->i2c->read_byte_data(CHIP_ID_ADDR)
                               .map_error(interface::i2c_error_to_string)
                               .map(std::to_integer<interface::I2c::Addr>);
        if (chip_id_res && chip_id_res.value() == ID) {  // 正常にBNO055が起動したことを確認
            timeout = false;
            break;
        }
    ...
    if (timeout) {
        return tl::make_unexpected(
            "BNO055 did not restart within timeout period (" + std::to_string(TIMEOUT_MS) +
            " ms)");
    }
...
auto ImuModel::on_read() -> tl::expected<
    ...
    auto transfer_res = this->i2c->transfer(messages).map_error(interface::i2c_error_to_string);
    if (!transfer_res) {
        return tl::make_unexpected(
            "Failed to read BNO055 measurements: I2C transfer error: " + transfer_res.error());
    }
    ...
    const auto gyro_scale = get_scale(VectorType::Gyroscope);
    ...
    const auto accel_scale = get_scale(VectorType::LinearAccel);
...
auto ImuModel::get_scale(VectorType type) -> double {
    switch (type) {
