...
auto impl::Pigpio::i2c_read_device(std::byte * buffer, const size_t length) const
    -> tl::expected<void, Error> {
    if (!this->i2c_handle) {
        return tl::unexpected(Error::NoHandle);
    }
    auto res = ::i2c_read_device(
        this->pi, this->i2c_handle.value(), reinterpret_cast<char *>(buffer), length);
