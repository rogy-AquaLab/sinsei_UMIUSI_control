...
class ImuSanity {
  public:
    struct Options {
        ...
        // ノルムの許容誤差。BNO055 の出力は厳密な単位ノルムではなく、正常時でも 0.8% ずれる
        // (2026-09-06 の実測 84502 サンプルで最悪 ||q|-1| = 0.00795)。一方いちばん軽い
        // 化けでも 0.106 離れている (known_issues A-1 の |q|=1.106)。両側から離すために
        // 0.03 を採る (正常側に 3.8 倍、化け側に 3.5 倍)。autonomy の既定は 0.01 のままで、
        // あちらは棄却しない運用なので問題になっていない
        double quat_tol = 0.03;
