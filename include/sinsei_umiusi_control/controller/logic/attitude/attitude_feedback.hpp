...
class AttitudeFeedback {
  private:
    ...
    static auto rotate_world_to_body(
        const Eigen::Quaterniond & attitude, const Eigen::Vector3d & vector)
        -> Eigen::Vector3d {
        const auto w = attitude.w();
        const auto x = attitude.x();
        const auto y = attitude.y();
  ...
  public:
    ...
    auto moment(
        ...
        const Eigen::Vector3d tilt_error_world{
            current_up_world.y() * target_up_world.z() -
                current_up_world.z() * target_up_world.y(),
            current_up_world.z() * target_up_world.x() -
                current_up_world.x() * target_up_world.z(),
            current_up_world.x() * target_up_world.y() -
                current_up_world.y() * target_up_world.x(),
        };
        const auto tilt_error_body =
            rotate_world_to_body(current_attitude, tilt_error_world);
